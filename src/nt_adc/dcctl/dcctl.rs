#[doc = "Reader of register DCCTL"]
pub type R = crate::R<u32, super::DCCTL>;
#[doc = "Writer for register DCCTL"]
pub type W = crate::W<u32, super::DCCTL>;
#[doc = "Register DCCTL `reset()`'s with value 0"]
impl crate::ResetValue for super::DCCTL {
    type Type = u32;
    #[inline(always)]
    fn reset_value() -> Self::Type {
        0
    }
}
#[doc = "DCMP interrupt generation mode\n\nValue on reset: 0"]
#[derive(Clone, Copy, Debug, PartialEq)]
pub enum CIM_A {
    #[doc = "0: multiple trigger mode"]
    MULTIPLE,
    #[doc = "1: single trigger mode"]
    SINGLE,
    #[doc = "2: multiple trigger mode with hysteresis"]
    MULTIPLEHYST,
    #[doc = "3: single trigger mode with hysteresis"]
    SINGLEHYST,
}
impl From<CIM_A> for u8 {
    #[inline(always)]
    fn from(variant: CIM_A) -> Self {
        match variant {
            CIM_A::MULTIPLE => 0,
            CIM_A::SINGLE => 1,
            CIM_A::MULTIPLEHYST => 2,
            CIM_A::SINGLEHYST => 3,
        }
    }
}
#[doc = "Reader of field `CIM`"]
pub type CIM_R = crate::R<u8, CIM_A>;
impl CIM_R {
    #[doc = r"Get enumerated values variant"]
    #[inline(always)]
    pub fn variant(&self) -> CIM_A {
        match self.bits {
            0 => CIM_A::MULTIPLE,
            1 => CIM_A::SINGLE,
            2 => CIM_A::MULTIPLEHYST,
            3 => CIM_A::SINGLEHYST,
            _ => unreachable!(),
        }
    }
    #[doc = "Checks if the value of the field is `MULTIPLE`"]
    #[inline(always)]
    pub fn is_multiple(&self) -> bool {
        *self == CIM_A::MULTIPLE
    }
    #[doc = "Checks if the value of the field is `SINGLE`"]
    #[inline(always)]
    pub fn is_single(&self) -> bool {
        *self == CIM_A::SINGLE
    }
    #[doc = "Checks if the value of the field is `MULTIPLEHYST`"]
    #[inline(always)]
    pub fn is_multiple_hyst(&self) -> bool {
        *self == CIM_A::MULTIPLEHYST
    }
    #[doc = "Checks if the value of the field is `SINGLEHYST`"]
    #[inline(always)]
    pub fn is_single_hyst(&self) -> bool {
        *self == CIM_A::SINGLEHYST
    }
}
#[doc = "Write proxy for field `CIM`"]
pub struct CIM_W<'a> {
    w: &'a mut W,
}
impl<'a> CIM_W<'a> {
    #[doc = r"Writes `variant` to the field"]
    #[inline(always)]
    pub fn variant(self, variant: CIM_A) -> &'a mut W {
        {
            self.bits(variant.into())
        }
    }
    #[doc = "multiple trigger mode"]
    #[inline(always)]
    pub fn multiple(self) -> &'a mut W {
        self.variant(CIM_A::MULTIPLE)
    }
    #[doc = "single trigger mode"]
    #[inline(always)]
    pub fn single(self) -> &'a mut W {
        self.variant(CIM_A::SINGLE)
    }
    #[doc = "multiple trigger mode with hysteresis"]
    #[inline(always)]
    pub fn multiple_hyst(self) -> &'a mut W {
        self.variant(CIM_A::MULTIPLEHYST)
    }
    #[doc = "single trigger mode with hysteresis"]
    #[inline(always)]
    pub fn single_hyst(self) -> &'a mut W {
        self.variant(CIM_A::SINGLEHYST)
    }
    #[doc = r"Writes raw bits to the field"]
    #[inline(always)]
    pub fn bits(self, value: u8) -> &'a mut W {
        self.w.bits = (self.w.bits & !0x03) | ((value as u32) & 0x03);
        self.w
    }
}
#[doc = "DCMP interrupt generation compare conditions\n\nValue on reset: 0"]
#[derive(Clone, Copy, Debug, PartialEq)]
pub enum CIC_A {
    #[doc = "0: result lower or equal COMP0"]
    LOW,
    #[doc = "1: result between COMP0 and COMP1 or equal any of them"]
    WINDOW,
    #[doc = "3: result higher or equal COMP1"]
    HIGH,
}
impl From<CIC_A> for u8 {
    #[inline(always)]
    fn from(variant: CIC_A) -> Self {
        match variant {
            CIC_A::LOW => 0,
            CIC_A::WINDOW => 1,
            CIC_A::HIGH => 3,
        }
    }
}
#[doc = "Reader of field `CIC`"]
pub type CIC_R = crate::R<u8, CIC_A>;
impl CIC_R {
    #[doc = r"Get enumerated values variant"]
    #[inline(always)]
    pub fn variant(&self) -> crate::Variant<u8, CIC_A> {
        use crate::Variant::*;
        match self.bits {
            0 => Val(CIC_A::LOW),
            1 => Val(CIC_A::WINDOW),
            3 => Val(CIC_A::HIGH),
            i => Res(i),
        }
    }
    #[doc = "Checks if the value of the field is `LOW`"]
    #[inline(always)]
    pub fn is_low(&self) -> bool {
        *self == CIC_A::LOW
    }
    #[doc = "Checks if the value of the field is `WINDOW`"]
    #[inline(always)]
    pub fn is_window(&self) -> bool {
        *self == CIC_A::WINDOW
    }
    #[doc = "Checks if the value of the field is `HIGH`"]
    #[inline(always)]
    pub fn is_high(&self) -> bool {
        *self == CIC_A::HIGH
    }
}
#[doc = "Write proxy for field `CIC`"]
pub struct CIC_W<'a> {
    w: &'a mut W,
}
impl<'a> CIC_W<'a> {
    #[doc = r"Writes `variant` to the field"]
    #[inline(always)]
    pub fn variant(self, variant: CIC_A) -> &'a mut W {
        unsafe { self.bits(variant.into()) }
    }
    #[doc = "result lower or equal COMP0"]
    #[inline(always)]
    pub fn low(self) -> &'a mut W {
        self.variant(CIC_A::LOW)
    }
    #[doc = "result between COMP0 and COMP1 or equal any of them"]
    #[inline(always)]
    pub fn window(self) -> &'a mut W {
        self.variant(CIC_A::WINDOW)
    }
    #[doc = "result higher or equal COMP1"]
    #[inline(always)]
    pub fn high(self) -> &'a mut W {
        self.variant(CIC_A::HIGH)
    }
    #[doc = r"Writes raw bits to the field"]
    #[inline(always)]
    pub unsafe fn bits(self, value: u8) -> &'a mut W {
        self.w.bits = (self.w.bits & !(0x03 << 2)) | (((value as u32) & 0x03) << 2);
        self.w
    }
}
#[doc = "Reader of field `CIE`"]
pub type CIE_R = crate::R<bool, bool>;
#[doc = "Write proxy for field `CIE`"]
pub struct CIE_W<'a> {
    w: &'a mut W,
}
impl<'a> CIE_W<'a> {
    #[doc = r"Sets the field bit"]
    #[inline(always)]
    pub fn set_bit(self) -> &'a mut W {
        self.bit(true)
    }
    #[doc = r"Clears the field bit"]
    #[inline(always)]
    pub fn clear_bit(self) -> &'a mut W {
        self.bit(false)
    }
    #[doc = r"Writes raw bits to the field"]
    #[inline(always)]
    pub fn bit(self, value: bool) -> &'a mut W {
        self.w.bits = (self.w.bits & !(0x01 << 4)) | (((value as u32) & 0x01) << 4);
        self.w
    }
}
#[doc = "DCMP output trigger mode\n\nValue on reset: 0"]
#[derive(Clone, Copy, Debug, PartialEq)]
pub enum CTM_A {
    #[doc = "0: multiple trigger mode"]
    MULTIPLE,
    #[doc = "1: single trigger mode"]
    SINGLE,
    #[doc = "2: multiple trigger mode with hysteresis"]
    MULTIPLEHYST,
    #[doc = "3: single trigger mode with hysteresis"]
    SINGLEHYST,
}
impl From<CTM_A> for u8 {
    #[inline(always)]
    fn from(variant: CTM_A) -> Self {
        match variant {
            CTM_A::MULTIPLE => 0,
            CTM_A::SINGLE => 1,
            CTM_A::MULTIPLEHYST => 2,
            CTM_A::SINGLEHYST => 3,
        }
    }
}
#[doc = "Reader of field `CTM`"]
pub type CTM_R = crate::R<u8, CTM_A>;
impl CTM_R {
    #[doc = r"Get enumerated values variant"]
    #[inline(always)]
    pub fn variant(&self) -> CTM_A {
        match self.bits {
            0 => CTM_A::MULTIPLE,
            1 => CTM_A::SINGLE,
            2 => CTM_A::MULTIPLEHYST,
            3 => CTM_A::SINGLEHYST,
            _ => unreachable!(),
        }
    }
    #[doc = "Checks if the value of the field is `MULTIPLE`"]
    #[inline(always)]
    pub fn is_multiple(&self) -> bool {
        *self == CTM_A::MULTIPLE
    }
    #[doc = "Checks if the value of the field is `SINGLE`"]
    #[inline(always)]
    pub fn is_single(&self) -> bool {
        *self == CTM_A::SINGLE
    }
    #[doc = "Checks if the value of the field is `MULTIPLEHYST`"]
    #[inline(always)]
    pub fn is_multiple_hyst(&self) -> bool {
        *self == CTM_A::MULTIPLEHYST
    }
    #[doc = "Checks if the value of the field is `SINGLEHYST`"]
    #[inline(always)]
    pub fn is_single_hyst(&self) -> bool {
        *self == CTM_A::SINGLEHYST
    }
}
#[doc = "Write proxy for field `CTM`"]
pub struct CTM_W<'a> {
    w: &'a mut W,
}
impl<'a> CTM_W<'a> {
    #[doc = r"Writes `variant` to the field"]
    #[inline(always)]
    pub fn variant(self, variant: CTM_A) -> &'a mut W {
        {
            self.bits(variant.into())
        }
    }
    #[doc = "multiple trigger mode"]
    #[inline(always)]
    pub fn multiple(self) -> &'a mut W {
        self.variant(CTM_A::MULTIPLE)
    }
    #[doc = "single trigger mode"]
    #[inline(always)]
    pub fn single(self) -> &'a mut W {
        self.variant(CTM_A::SINGLE)
    }
    #[doc = "multiple trigger mode with hysteresis"]
    #[inline(always)]
    pub fn multiple_hyst(self) -> &'a mut W {
        self.variant(CTM_A::MULTIPLEHYST)
    }
    #[doc = "single trigger mode with hysteresis"]
    #[inline(always)]
    pub fn single_hyst(self) -> &'a mut W {
        self.variant(CTM_A::SINGLEHYST)
    }
    #[doc = r"Writes raw bits to the field"]
    #[inline(always)]
    pub fn bits(self, value: u8) -> &'a mut W {
        self.w.bits = (self.w.bits & !(0x03 << 8)) | (((value as u32) & 0x03) << 8);
        self.w
    }
}
#[doc = "DCMP output trigger compare conditions\n\nValue on reset: 0"]
#[derive(Clone, Copy, Debug, PartialEq)]
pub enum CTC_A {
    #[doc = "0: result lower or equal COMP0"]
    LOW,
    #[doc = "1: result between COMP0 and COMP1 or equal any of them"]
    WINDOW,
    #[doc = "3: result higher or equal COMP1"]
    HIGH,
}
impl From<CTC_A> for u8 {
    #[inline(always)]
    fn from(variant: CTC_A) -> Self {
        match variant {
            CTC_A::LOW => 0,
            CTC_A::WINDOW => 1,
            CTC_A::HIGH => 3,
        }
    }
}
#[doc = "Reader of field `CTC`"]
pub type CTC_R = crate::R<u8, CTC_A>;
impl CTC_R {
    #[doc = r"Get enumerated values variant"]
    #[inline(always)]
    pub fn variant(&self) -> crate::Variant<u8, CTC_A> {
        use crate::Variant::*;
        match self.bits {
            0 => Val(CTC_A::LOW),
            1 => Val(CTC_A::WINDOW),
            3 => Val(CTC_A::HIGH),
            i => Res(i),
        }
    }
    #[doc = "Checks if the value of the field is `LOW`"]
    #[inline(always)]
    pub fn is_low(&self) -> bool {
        *self == CTC_A::LOW
    }
    #[doc = "Checks if the value of the field is `WINDOW`"]
    #[inline(always)]
    pub fn is_window(&self) -> bool {
        *self == CTC_A::WINDOW
    }
    #[doc = "Checks if the value of the field is `HIGH`"]
    #[inline(always)]
    pub fn is_high(&self) -> bool {
        *self == CTC_A::HIGH
    }
}
#[doc = "Write proxy for field `CTC`"]
pub struct CTC_W<'a> {
    w: &'a mut W,
}
impl<'a> CTC_W<'a> {
    #[doc = r"Writes `variant` to the field"]
    #[inline(always)]
    pub fn variant(self, variant: CTC_A) -> &'a mut W {
        unsafe { self.bits(variant.into()) }
    }
    #[doc = "result lower or equal COMP0"]
    #[inline(always)]
    pub fn low(self) -> &'a mut W {
        self.variant(CTC_A::LOW)
    }
    #[doc = "result between COMP0 and COMP1 or equal any of them"]
    #[inline(always)]
    pub fn window(self) -> &'a mut W {
        self.variant(CTC_A::WINDOW)
    }
    #[doc = "result higher or equal COMP1"]
    #[inline(always)]
    pub fn high(self) -> &'a mut W {
        self.variant(CTC_A::HIGH)
    }
    #[doc = r"Writes raw bits to the field"]
    #[inline(always)]
    pub unsafe fn bits(self, value: u8) -> &'a mut W {
        self.w.bits = (self.w.bits & !(0x03 << 10)) | (((value as u32) & 0x03) << 10);
        self.w
    }
}
#[doc = "Reader of field `CTE`"]
pub type CTE_R = crate::R<bool, bool>;
#[doc = "Write proxy for field `CTE`"]
pub struct CTE_W<'a> {
    w: &'a mut W,
}
impl<'a> CTE_W<'a> {
    #[doc = r"Sets the field bit"]
    #[inline(always)]
    pub fn set_bit(self) -> &'a mut W {
        self.bit(true)
    }
    #[doc = r"Clears the field bit"]
    #[inline(always)]
    pub fn clear_bit(self) -> &'a mut W {
        self.bit(false)
    }
    #[doc = r"Writes raw bits to the field"]
    #[inline(always)]
    pub fn bit(self, value: bool) -> &'a mut W {
        self.w.bits = (self.w.bits & !(0x01 << 12)) | (((value as u32) & 0x01) << 12);
        self.w
    }
}
#[doc = "Reader of field `CHNL`"]
pub type CHNL_R = crate::R<u8, u8>;
#[doc = "Write proxy for field `CHNL`"]
pub struct CHNL_W<'a> {
    w: &'a mut W,
}
impl<'a> CHNL_W<'a> {
    #[doc = r"Writes raw bits to the field"]
    #[inline(always)]
    pub unsafe fn bits(self, value: u8) -> &'a mut W {
        self.w.bits = (self.w.bits & !(0x1f << 16)) | (((value as u32) & 0x1f) << 16);
        self.w
    }
}
impl R {
    #[doc = "Bits 0:1 - DCMP interrupt generation mode"]
    #[inline(always)]
    pub fn cim(&self) -> CIM_R {
        CIM_R::new((self.bits & 0x03) as u8)
    }
    #[doc = "Bits 2:3 - DCMP interrupt generation compare conditions"]
    #[inline(always)]
    pub fn cic(&self) -> CIC_R {
        CIC_R::new(((self.bits >> 2) & 0x03) as u8)
    }
    #[doc = "Bit 4 - Enable DCMP interrupt generation"]
    #[inline(always)]
    pub fn cie(&self) -> CIE_R {
        CIE_R::new(((self.bits >> 4) & 0x01) != 0)
    }
    #[doc = "Bits 8:9 - DCMP output trigger mode"]
    #[inline(always)]
    pub fn ctm(&self) -> CTM_R {
        CTM_R::new(((self.bits >> 8) & 0x03) as u8)
    }
    #[doc = "Bits 10:11 - DCMP output trigger compare conditions"]
    #[inline(always)]
    pub fn ctc(&self) -> CTC_R {
        CTC_R::new(((self.bits >> 10) & 0x03) as u8)
    }
    #[doc = "Bit 12 - Enable DCMP output trigger"]
    #[inline(always)]
    pub fn cte(&self) -> CTE_R {
        CTE_R::new(((self.bits >> 12) & 0x01) != 0)
    }
    #[doc = "Bits 16:20 - ADC channel selection"]
    #[inline(always)]
    pub fn chnl(&self) -> CHNL_R {
        CHNL_R::new(((self.bits >> 16) & 0x1f) as u8)
    }
}
impl W {
    #[doc = "Bits 0:1 - DCMP interrupt generation mode"]
    #[inline(always)]
    pub fn cim(&mut self) -> CIM_W {
        CIM_W { w: self }
    }
    #[doc = "Bits 2:3 - DCMP interrupt generation compare conditions"]
    #[inline(always)]
    pub fn cic(&mut self) -> CIC_W {
        CIC_W { w: self }
    }
    #[doc = "Bit 4 - Enable DCMP interrupt generation"]
    #[inline(always)]
    pub fn cie(&mut self) -> CIE_W {
        CIE_W { w: self }
    }
    #[doc = "Bits 8:9 - DCMP output trigger mode"]
    #[inline(always)]
    pub fn ctm(&mut self) -> CTM_W {
        CTM_W { w: self }
    }
    #[doc = "Bits 10:11 - DCMP output trigger compare conditions"]
    #[inline(always)]
    pub fn ctc(&mut self) -> CTC_W {
        CTC_W { w: self }
    }
    #[doc = "Bit 12 - Enable DCMP output trigger"]
    #[inline(always)]
    pub fn cte(&mut self) -> CTE_W {
        CTE_W { w: self }
    }
    #[doc = "Bits 16:20 - ADC channel selection"]
    #[inline(always)]
    pub fn chnl(&mut self) -> CHNL_W {
        CHNL_W { w: self }
    }
}
